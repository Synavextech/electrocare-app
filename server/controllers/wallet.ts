...
export const redeemPoints = async (req: Request, res: Response) => {
  try {
    const user = await getUserById(req.user!.id);
    if (!user?.wallet || user.wallet.points < 10) return res.status(400).json({ error: 'Insufficient points' });
...
export const redeemElectroCoins = async (req: Request, res: Response) => {
  try {

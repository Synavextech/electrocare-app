...
async function seed() {
    ...
    try {
        ...
        for (const [code, id] of Object.entries(shopIdMap)) {
            ...
            technicians.push({ name: `Tech ${cityName} A`, email: `tech.${cityName}.a@electrocare.com`, phone: '0712345678', category: 'Hardware', subCategory: 'Screen/LCD', shopId: id, rating: 4.8, description: 'Expert in mobile hardware' }, { name: `Tech ${cityName} B`, email: `tech.${cityName}.b@electrocare.com`, phone: '0723456789', category: 'Software', subCategory: 'OS/Firmware', shopId: id, rating: 4.7, description: 'Software troubleshooting specialist' });
        }
        for (const tech of technicians) {
            const { error } = await supabase.from('Technicians').upsert(tech, { onConflict: 'email' });
            if (error)
                console.error(`❌ Error seeding technician ${tech.name}:`, error.message);
            else
                console.log(`✅ Seeded Technician: ${tech.name}`);
        }
        ...
        for (const dp of deliveryPersonnel) {
            ...
            if (error)
                console.error(`❌ Error seeding delivery: ${dp.name}:`, error.message);
            else
                console.log(`✅ Seeded Delivery: ${dp.name}`);
        }
        // 4. Seed Device Types
        ...
        for (const type of deviceTypes) {
            ...
            if (error)
                console.error(`❌ Error seeding device type ${type.name}:`, error.message);
            else if (data) {
        ...
        for (const model of models) {
            ...
            if (typeId) {
                const { error } = await supabase.from('DeviceModels').upsert({
                    name: model.name,
                    brand: model.brand,
                    deviceTypeId: typeId
...
//# sourceMappingURL=full_seed.js.map